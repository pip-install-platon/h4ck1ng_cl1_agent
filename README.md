...
| **Команда «зависла» в stream/classic без ошибки** | Часто процесс **ждал stdin** (наследование TTY). В новых версиях copilot захват использует **`stdin=/dev/null`**; если утилита реально нужен ввод или полноценный TTY (**less**, меню) — **`COPILOT_FG_MODE=tty`** или запуск внешнего терминала. **`exploitdb` / `searchsploit`** с опциями вроде зеркала/редактора могут блокироваться без TTY — проверь флаги (`-h`) или режим tty. |
